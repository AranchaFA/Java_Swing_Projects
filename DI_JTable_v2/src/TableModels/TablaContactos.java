...
public class TablaContactos extends AbstractTableModel {
    ...
    public int removeRows(int[] rowsOrdenados) {
        // Losos ÍNDICES tienen que estar ORDENADOS
        Arrays.sort(rowsOrdenados);
        int contador=0;
        for (int row : rowsOrdenados) {
            // Creamos el objeto contacto a borrar
            //Contacto contactoABorrar = generarContacto(row);
            // Borramos en la lista del controlador el contacto
            boolean borrado=removeRow(row-contador);
            /*boolean borrado=LogicaNegocio.removeContacto(contactoABorrar);*/
            if (borrado) {
                contador++;
            }
        }
    ...
    public boolean removeRow(int row) {
        ...
        /*Contacto contactoABorrar = generarContacto(row);
        // Borramos en la lista del controlador el contacto
        return LogicaNegocio.removeContacto(contactoABorrar);*/
    }
